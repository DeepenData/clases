---
{title: "Minado de conociiento biologico"
date: 2020-11-12,
...
```{r message=FALSE, warning=FALSE}
data0 %>% names %>% str_extract('GSE\\d+.GPL\\d+') %>% set_names(data,.) -> data # TODO: what?
...
tsv.files.path %>%
      map(function(files.path00){readr::read_delim(files.path00, delim = '\t', skip = 0,col_names = TRUE)}) -> df_list # Genera una lista de datafrmaes que corresponde a la salida de los TSV
